...
1. 研究目的と成功条件を確認する。
2. 先行研究の根拠、取得元、アクセス条件、未解決点を `metadata.yaml`、`idea_notes.md`、`paper.md` から確認する。
...
12. 合法的に取得できる DOI、PMID、PMCID、`paper_url`、`pdf_url`、`code_url` が分かっている場合だけ、`prior-research-downloader` を使う。
13. `prior-research-downloader` は通常、取得後に `paper.md` と `source.md` の作成まで実行する前提で扱う。
14. `prior-research-ingester` 単独利用は、`--no-ingest` 後、手動PDF配置後、または `paper.md` / `source.md` 欠落時に限る。
15. `paper.md` が作成されるまでは、その論文を引用根拠として本文に断定を書かない。
16. 論文各セクションの下書きを作る。
17. `manuscript/references.md` に番号付き参考文献リストを作る。
18. `introduction.md`, `methods.md`, `results.md`, `discussion.md` の引用箇所に対応する引用番号を書く。
19. 本文中の引用番号と `references.md` の番号が一対一に対応しているか確認する。
20. 数式はVS Code Markdownプレビュー互換の記法で書く。
21. 根拠がない主張には「根拠不足」と書く。
22. 必要な表、図、集約結果が `results/` にない場合は、原稿で作ったことにせず、`02-research-executor` または `03-review-gate` に戻す不足事項として記録する。
23. 投稿準備へ進む前に `03-review-gate` を呼ぶ。
...
- 引用に必要な論文の取得またはingestが必要な場合は `prior-research-downloader` または `prior-research-ingester` に戻る。
- 原稿だけの修正ならこのskillに戻る。
