#!/usr/bin/env python3
"""先行研究PDFを取得し、Markdown成果物まで作成する。

このファイルの目的:
`prior_research/<paper_id>/metadata.yaml` に書かれた DOI、PMID、PMCID、
PDF URL、論文ページURL、公開コードURLを読み、合法的に取得できる公開PDFを
`paper.pdf` として保存します。取得できたPDFは `paper.md` に変換し、公開コードは
cloneせず `gitingest` で `source.md` に変換します。

重要な方針:
paywall、ログイン、token、private repository、機関認証、自動アクセス防御は回避しません。
`paper.md` は必ず `paper.pdf` からだけ作り、HTMLやabstractからは作りません。
"""
...
USER_AGENT = "ResearchCopilotMVP/0.1"
...
SCRIPT_RELATIVE_PATH = ".agents/skills/utilities/prior-research-downloader/scripts/download_prior_research.py"
...
def download_pdf(pdf_url: str, destination: Path, force: bool) -> str:
    ...
    try:
        ...
        if is_ssl_cert_verification_error(error.reason):
...
def is_ssl_cert_verification_error(reason: object) -> bool:
    try:
        import ssl  # noqa: PLC0415
    except ImportError:
        return reason.__class__.__name__ == "SSLCertVerificationError"
...
def ingest_script_candidates() -> list[Path]:
    script_dir = Path(__file__).resolve().parent
    utilities_dir = script_dir.parent.parent
    return [
        script_dir / "ingest_prior_research.py",
        utilities_dir / "prior-research-ingester" / "scripts" / "ingest_prior_research.py",
    ]


def load_ingest_module() -> object:
    checked_paths: list[str] = []
    for script_path in ingest_script_candidates():
    ...
    raise RuntimeError("ingest scriptを読み込めない。確認したpath: " + ", ".join(checked_paths))
